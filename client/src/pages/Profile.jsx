        ...
        return (
                <div className="container">
                    <div className="row">
                        ...
                        {userCampaigns && userCampaigns.map(data => <CampaignCard key={data._id} title={data.name} created={data.date} dm={data.DM} campaignId={data._id} />)}
                        </div>
                        <div className="col s12 m6" style={{height: "80vh", overflow: "auto"}}>
                        <h2 style={{color: "#fff"}}>Characters</h2>
                        {userCharacters && userCharacters.map(data => <CampaignCard key={data._id} title={data.name} created={data.date} characterId={data._id} />)}
                        </div>
                    </div>
                </div>
        )
